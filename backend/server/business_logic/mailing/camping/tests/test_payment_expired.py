...
from server.business_logic.mailing.camping import PaymentExpiredMail
...
class PaymentExpiredMailTestCase(TestCase):
    ...
    def test_send(
        ...
        subject = str(PaymentExpiredMail._subject_template)
        ...
        with self.assertLogs(logger=logger.name, level='DEBUG') as context:
            PaymentExpiredMail.send(reservation=self.reservation)

            expected_log = get_formatted_log(
                msg=PaymentExpiredMail._logger_message,
                level='INFO',
                logger=logger,
            )
