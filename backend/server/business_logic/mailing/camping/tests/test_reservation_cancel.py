...
class ReservationCancelMailTestCase(TestCase):
    ...
    def test_send(
        ...
        subject = str(ReservationCancelMail._subject_template)
        ...
        with self.assertLogs(logger=logger.name, level='DEBUG') as context:
            ReservationCancelMail.send(reservation=self.reservation)

            expected_log = get_formatted_log(
                msg=ReservationCancelMail._logger_message,
                level='INFO',
                logger=logger,
            )
